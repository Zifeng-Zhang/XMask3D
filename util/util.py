...
def cosine_learning_rate(base_lr, curr_iter, max_iter):
    ...
    lr = base_lr * 0.5 * (1 + math.cos(math.pi * curr_iter / max_iter))
    return lr
...
def intersectionAndUnionGPU(output, target, K, ignore_indexs=[255]):
    ...
    for ignore_index in ignore_indexs:
...
def get_palette(num_cls=21, colormap="scannet"):
    ...
    elif colormap == "scannet_200":
        scannet_palette = []
        for _, value in SCANNET_COLOR_MAP_200.items():
            scannet_palette.append(np.array(value))
        palette = np.concatenate(scannet_palette)
