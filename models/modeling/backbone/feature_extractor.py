...
class FeatureExtractorBackbone(Backbone):
    def __init__(
        ...
        if self._slide_training:
            assert (
                self._slide_inference
            ), "slide training must be used with slide inference"
        ...
        for indices in self._sorted_grouped_indices:
            ...
            if name not in out_features:
                continue
            assert (
                name not in self._out_feature_strides
    ...
    def single_forward(self, img, imp_condition):
        ...
        features = self.feature_extractor(dict(img=img), imp_condition)
        ...
        for name, indices in zip(self._out_features, self._sorted_grouped_indices):
            ...
            for idx in indices:
                ...
                restored_feature = F.interpolate(
                    features[idx],
                    size=(
                        input_image_size[-2] // stride,
                        input_image_size[-1] // stride,
                    ),
                )
    ...
    def slide_forward(self, img, imp_condition):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                assert crop_img.shape[-2:] == (
                    h_crop,
                    w_crop,
                ), f"{crop_img.shape} from {img.shape}"

                crop_features = self.single_forward(crop_img, imp_condition)
    ...
    def forward(self, img, imp_condition):

        if (self.training and not self._slide_training) or not self._slide_inference:
            return self.single_forward(img, imp_condition)
        else:
            """"""
            return self.slide_forward(img, imp_condition)
